import { FileObject } from 'zarm/types/file-picker/interface'
...
function UserInfo() {
  ...
  // 获取图片回调
  const handleSelect = (file: FileObject | FileObject[]) => {
    console.log('file', file)
    if (file && (file as FileObject).file.size > 200 * 1024) {
      Toast.show('上传头像不得超过 200 KB！！')
      return
    }
    let formData = new FormData()
    formData.append('file', (file as FileObject).file)
        ...
        <div className={s.item}>
          ...
          <div className={s.signature}>
            <Input
              ...
              value={signature}
              placeholder="请输入个性签名"
              onChange={(value:ChangeEvent<HTMLInputElement>) => setSignature(value.target.value)}
            />
          </div>
        </div>
